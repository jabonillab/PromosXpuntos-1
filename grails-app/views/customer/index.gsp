		...
		<div id="list-customer" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="identification" title="${message(code: 'customer.identification.label', default: 'Identification')}" />
					
						<g:sortableColumn property="name" title="${message(code: 'customer.name.label', default: 'Name')}" />
					
						<g:sortableColumn property="contractNumber" title="${message(code: 'customer.contractNumber.label', default: 'Contract Number')}" />
				...
				<g:each in="${customerInstanceList}" status="i" var="customerInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${customerInstance.id}">${fieldValue(bean: customerInstance, field: "identification")}</g:link></td>
					
						<td>${fieldValue(bean: customerInstance, field: "name")}</td>
					
						<td>${fieldValue(bean: customerInstance, field: "contractNumber")}</td>
